...
const InviteFriendsScreen = () => {
  ...
  const fetchReferralData = async () => {
    try {
      ...
      const statsResponse = await fetch(
        `${BASE_URL}/users/${userId}/referral-stats`
      );
  ...
  const fetchInviteCode = async () => {
    try {
      ...
      const response = await fetch(`${BASE_URL}/users/${userId}/invite-code`);
  ...
  // 添加扫描处理函数
  const handleScan = () => {
    router.push('/qrcodescanner' as Href);
  };
  ...
  const handleShare = async (platform: string) => {
    ...
    try {
      switch (platform) {
        case 'Facebook':
          await Linking.openURL(
            `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(
              `https://zenwaves.app/invite/${invitationCode}`
            )}`
          );
          break;
        ...
        case 'Email':
          await Linking.openURL(
            `mailto:?subject=Join%20me%20on%20ZenWaves&body=${encodeURIComponent(
              shareText
            )}`
          );
          break;
        ...
        case 'Instagram':
          ...
          Alert.alert(
            'Notice',
            'Text copied to clipboard. You can now paste it in Instagram.'
          );
          break;
      ...
      switch (platform) {
        ...
        default:
          Alert.alert(
            'Error',
            `Could not open ${platform}. Please make sure the app is installed.`
          );
      }
    }
  };
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Text style={[styles.subTitle, { marginTop: 30 }]}>
        Copy this code and share it with friends to join ZenWaves.
      </Text>
      <Text style={[styles.label, { marginTop: 20 }]}>Invitation Code:</Text>
      <View
        style={[
          styles.codeContainer,
          { width: '80%', alignSelf: 'center', marginVertical: 15 },
        ]}
      >
      ...
      <TouchableOpacity
        style={[styles.qrButton, { marginTop: 15, marginBottom: 20 }]}
      ...
      {/* 分享邀请链接 */}
      <Text style={[styles.shareTitle, { marginTop: 30, marginBottom: 20 }]}>
      ...
      >
        <Text style={styles.explanationTitle}>
          Instructions & Tier Explanation:
        </Text>
        <Text style={styles.explanationText}>
          Earn More with Direct and Team Referrals. Unlock exclusive rewards by
          inviting more friends to ZenWaves:
        </Text>

        <Text style={styles.sectionHeader}>Direct Referral Rewards:</Text>
        <Text style={[styles.explanationText, { marginVertical: 10 }]}>
          • Refer 1-4 users and earn 15% of their subscription fees and creator
          top-up charges.
        </Text>
        <Text style={styles.explanationText}>
          • Refer 5-9 users with a minimum personal monthly sales of $220 to
          earn 18% of their subscription fees and creator top-up charges.
        </Text>
        <Text style={styles.explanationText}>
          • Refer 10 or more users with a minimum personal monthly sales of $550
          to earn 20% of their subscription fees and creator top-up charges.
        </Text>

        <Text style={styles.sectionHeader}>Team Performance Rewards:</Text>
        <Text style={[styles.explanationText, { marginVertical: 10 }]}>
          • Achieve a total monthly team sales of $220 to earn 3% of total team
          sales, including both subscriptions and top-up charges.
        </Text>
        <Text style={[styles.explanationText, { marginVertical: 10 }]}>
          • Achieve a total monthly team sales of $550 to earn 5% of total team
          sales, including both subscriptions and top-up charges.
        </Text>
        <Text style={[styles.explanationText, { marginVertical: 10 }]}>
          • Achieve a total monthly team sales of $1,100 to earn 7% of total
          team sales, including both subscriptions and top-up charges.
        </Text>
        <Text style={[styles.explanationText, { marginVertical: 10 }]}>
          • Achieve a total monthly team sales of $2,200 to earn 10% of total
          team sales, including both subscriptions and top-up charges.
        </Text>
      </View>
