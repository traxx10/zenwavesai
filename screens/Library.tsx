...
export default function LibraryScreen() {
  ...
  const params = useLocalSearchParams<{ draft_list_id: string }>();
  ...
  const handleConfirmSelection = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/users/${userId}/draft-list/${draft_list_id}/music-library/${musicIds}/add-music`,
        {
          method: 'POST',
          headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to add music to draft');
      }
